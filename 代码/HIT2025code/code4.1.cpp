...
int waypoint_num;
double err_without_z, err_z_takeoff, det_height,takeoff_height,PUT_height,FIND_height;
int wp_index = 0;
double tar_d1=0.15,tar_d2=0.2,tar_d3=0.25;

position pos_tar_1,pos_tar_2,pos_tar_3;
// 飞行状态初始化
...
int main(int argc, char **argv) {
    ...
    nh.getParam("FIND_height", FIND_height);   
    ...
    while (ros::ok()) {
        ...
        ROS_INFO("FIND");
            ...
            {
                // 切换到下一个目标点并发布运动指令//已经决定重复多次绕圈，具体路径规划仍待商榷
                if (wp_index <= 8) {
            ...
            if(fabs(current_d - tar_d1)<0.01)//加是否正确识别到序号桶的标志
            {
                pos_tar_1.x = local_pos.x + current_guidance.x;
                pos_tar_1.y = local_pos.y + current_guidance.y;
            }
            else if(fabs(current_d - tar_d2)<0.01)
            {
               pos_tar_2.x = local_pos.x + current_guidance.x;
                pos_tar_2.y = local_pos.y + current_guidance.y;
            }
            else if (fabs(current_d - tar_d3)<0.01)
            {
               pos_tar_3.x = local_pos.x + current_guidance.x;
                pos_tar_3.y = local_pos.y + current_guidance.y;
            }
//识别到三个桶，不等时间记满，直接切状态
            if (ros::Time::now() - start_time > ros::Duration(max_wait_time[6])) {//要对识别到两个桶，一个桶不同情况进行不同的状态切换
                ROS_ERROR("waypoint %d : FIND Timeout", wp_index);
                ms = GO_TO_TAR_1;
                //fail_find_tar = true;//如果没找到就强制原地投放，即进PUT_1后直接PUT_2 
                //wp_index = ; // 确保返回状态时使用正确的航点索引
                start_time = ros::Time::now();
            }
            break;
//要对识别到三个桶，两个桶，一个桶不同情况进行不同的状态切换

//从321顺序遍历，写在GOTOTAR状态中，投完一个将其标志变为false,下一个点再次便利

        }

        //假设桶位置未知
        case GO_TO_TAR_1://特殊情况，标志都为f，进入PUT
        {
            manual_pos_eigen = Eigen::Vector3d(pos_tar_3.x,pos_tar_3.y, takeoff_height);//具体投哪个桶取决于识别到桶的个数
            expect_pos_eigen = start_q_eigen * manual_pos_eigen;
            expect_pos.pose.position.x = expect_pos_eigen(0) ;
            expect_pos.pose.position.y = expect_pos_eigen(1) ;
        ...
        case PUT_AIM_1:
        {
            manual_pos_eigen = Eigen::Vector3d(current_guidance.x, current_guidance.y, PUT_height);
            expect_pos_eigen = start_q_eigen * manual_pos_eigen;
            expect_pos.pose.position.x = expect_pos_eigen(0) + start_pos.pose.position.x;
            expect_pos.pose.position.y = expect_pos_eigen(1) + start_pos.pose.position.y;
            expect_pos.pose.position.z = PUT_height + start_pos.pose.position.z;
