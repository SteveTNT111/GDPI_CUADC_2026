...
void guidance_cb(const geometry_msgs::Point::ConstPtr& msg) {
    ...
    if (current_d != 0) {
        ROS_INFO("Received guidance_local: x=%.2f, y=%.2f, diameter_cm=%.2f", 
                      msg->x, -msg->y, msg->z);
    }
...
void go_to_pub(double x, double y, double z) {
    ...
    ROS_INFO("expect_pos: [%.2f, %.2f, %.2f]", expect_pos.pose.position.x, expect_pos.pose.position.y, expect_pos.pose.position.z);
...
int main(int argc, char **argv) {
    ...
    while (ros::ok()) {
        ...
        case GO_TO_PUT://投水高度和寻找筒的高度和侦察高度？？？
        {
            ...
            expect_pos.pose.position.z = expect_pos_eigen(2) + start_pos.pose.position.z;
            // 发布目标位置
        ...
        case TAR_FIND:
        {
            ...
            {
                pos_tar[2].x = local_pos.pose.position.x + guidance_global(0);
            ...
            {
                pos_tar[3].x = local_pos.pose.position.x + guidance_global(0);
        ...
        case PUT_AIM_1:
        {
            ...
            guidance_local(2) = 0;
            guidance_global = start_q_eigen * guidance_local;
            expect_pos.pose.position.x = local_pos.pose.position.x + guidance_global(0);
            expect_pos.pose.position.y = local_pos.pose.position.y + guidance_global(1);
            expect_pos.pose.position.z = put_height + start_pos.pose.position.z;
            // 发布目标位置
            ROS_INFO_THROTTLE(0.5, "PUT_AIM_1\n guidance_local: [%.2f, %.2f, %.2f]", guidance_local(0), guidance_local(1), guidance_local(2)); 
            go_to_pub(expect_pos.pose.position.x, expect_pos.pose.position.y, expect_pos.pose.position.z);
            // 依靠图像检查是否到达航点
            if (is_guidance_valid && is_stable_arrival(guidance_local, 0.05, 0.5)) {
                ms = PUT_1;
                start_time = ros::Time::now();
            }
        ...
        case PUT_AIM_2:
        {
            ...
            expect_pos.pose.position.z = put_height + start_pos.pose.position.z;
            // 发布目标位置
            ROS_INFO_THROTTLE(0.5, "PUT_AIM_2\n guidance_local: [%.2f, %.2f, %.2f]", 
