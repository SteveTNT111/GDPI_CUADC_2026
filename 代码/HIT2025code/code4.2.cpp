...
bool is_valid_guidance(const geometry_msgs::Point& guidance) {//加时间约束，满足模长条条件达到一定时间才表示真正瞄准了
...
void diameter_cb(const std_msgs::Float64 msg) {
    // 处理直径信息
    current_d = msg.data;
    ROS_INFO("Received diameter: current_d = %f", current_d);
}
...
int wp_index = 0;
double max_wait_time[11];

double tar_d1,tar_d2,tar_d3;
...
int main(int argc, char **argv) {
    ...
    nh.getParam("tar_d1", tar_d1);
    nh.getParam("tar_d2", tar_d2);
    nh.getParam("tar_d3", tar_d3);
    
    ros::Subscriber guidance_sub;
    guidance_sub = nh.subscribe<geometry_msgs::Point>("/h_detect/guidance_vector", 10, guidance_cb);//获取引导向量
    ros::Subscriber diameter_sub;
    diameter_sub = nh.subscribe("/h_detect/diameter", 10, diameter_cb);
    ...
    while (ros::ok()) {
        ...
        case PUT_AIM_1:
        {
            manual_pos_eigen = Eigen::Vector3d(current_guidance.x, current_guidance.y, det_height);
            expect_pos_eigen = start_q_eigen * manual_pos_eigen;
            expect_pos.pose.position.x = expect_pos_eigen(0) + start_pos.pose.position.x;
            expect_pos.pose.position.y = expect_pos_eigen(1) + start_pos.pose.position.y;
            expect_pos.pose.position.z = det_height + start_pos.pose.position.z;
