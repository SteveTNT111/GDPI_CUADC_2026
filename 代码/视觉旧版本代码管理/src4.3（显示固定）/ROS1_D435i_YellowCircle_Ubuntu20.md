...
- `yolo_detector_node.py`: 加载 Ultralytics YOLO 模型, 对 D435i 彩色图做目标检测并显示/发布标注图。
- `competition_task_node.py`: 根据比赛场地规则和 YOLO 输出, 生成投放区/灾情侦察区任务目标。
...
YOLO 节点默认不启动。第一次使用前需要在 Ubuntu 上安装依赖, 并确认模型文件存在：
...
`depth_max_valid_m` 现在只表示“可信范围上限”, 不再控制是否显示。超过这个范围时 OpenCV 仍会显示距离, 但测距来源会带 `*`, 例如 `diameter*`、`depth*`、`calibrated*`。`*` 表示这个值超出了可信范围, 但仍在显示范围内。

如果希望超过 2m 也继续显示, 可以调大显示范围：

```bash
roslaunch d435i_yellow_circle_detector d435i_yellow_circle.launch show_window:=true enable_record:=true depth_max_display_m:=3.00
```

如果目标离相机小于约 16cm, 当前版本默认不显示距离。判断方法不是只看深度值, 还会看检测圆的像素半径。默认：

```bash
close_radius_px:=42.0
```

当画面中 `r` 大于等于 `42px` 时, 程序认为目标过近, `depth_source` 会显示为 `too_close`, OpenCV 不显示 `x/y/z/d` 距离。如果你的实际画面在 16cm 时半径不是 42px, 可以调整：

```bash
roslaunch d435i_yellow_circle_detector d435i_yellow_circle.launch show_window:=true enable_record:=true close_radius_px:=45.0
```
...
```text
/yolo/annotated_image sensor_msgs/Image, bgr8
/yolo/detection       d435i_yellow_circle_detector/YoloDetection
/yolo/detections      d435i_yellow_circle_detector/YoloDetections
```

`/yolo/detection` 会输出 YOLO 置信度最高目标的类别、检测框、中心点像素、中心点深度以及相机坐标系下的 `x/y/z/d`。OpenCV 的 `YOLO Detection` 窗口会画出目标中心点, 并显示类似：

```text
class conf x=... y=... z=... d=...m
```
...
```text
/competition/target d435i_yellow_circle_detector/MissionTarget
```

`/competition/target` 会根据 `mission_stage` 输出当前任务目标：

```text
mission_stage=drop      投放区, 识别 15/20/25cm 白色圆筒, 输出 A 区半径和 B 区半径
mission_stage=disaster  灾情侦察区, 优先识别 12x12cm 危险化学品标识, 其次识别 20cm 白色圆筒
mission_stage=auto      只按类别名粗分类, 不能替代全局导航状态
```

注意：D435i 只能输出目标相对相机的位置, 无法单独判断无人机已经飞到全场的 30m 投放区或 55m 灾情侦察区。这个判断需要飞控/导航状态提供, 本节点用 `mission_stage` 参数接收当前任务阶段。
...
<arg name="yolo_model_path" default="/home/lab/model/best.pt" />
...
<param name="model_path" value="/home/lab/model/best.pt" />
...
```xml
<param name="mission_stage" value="drop" />
...
如果 NUC 没有 NVIDIA GPU, 保持 `yolo_device:=cpu`。如果已正确安装 CUDA 版 PyTorch, 可以尝试 `yolo_device:=0`。
...
<param name="depth_max_display_m" value="2.00" />
<param name="depth_disagreement_ratio" value="0.35" />
<param name="close_radius_px" value="42.0" />
...
近距离场景下 D435/D435i 深度图可能不稳定。当前默认认为 `0.05m ~ 0.35m` 是可信测试范围, 但显示范围是 `0.01m ~ 2.00m`。如果所有测距结果都超出可信范围但仍在显示范围内, OpenCV 会显示距离并在来源后加 `*`。
