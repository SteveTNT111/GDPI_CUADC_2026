...
如果 OpenCV 预览窗口卡顿，可以降低显示尺寸：

```bash
roslaunch d435i_yellow_circle_detector d435i_yellow_circle.launch show_window:=true display_scale:=0.5
```

如果 USB3.2 稳定，也可以尝试更高帧率：

```bash
roslaunch d435i_yellow_circle_detector d435i_yellow_circle.launch show_window:=true fps:=30
```
...
- `fps=15`：当前默认值，USB3.2 下比 6 FPS 更流畅；如果仍然不稳，可运行时改成 `fps:=6`。
- `display_scale=1.0`：OpenCV 窗口显示比例；卡顿时可用 `display_scale:=0.5` 降低 GUI 压力。
...
Actual streams: color=640x480 bgr8@15 depth=640x480 format.z16@15
D435i started...
```
