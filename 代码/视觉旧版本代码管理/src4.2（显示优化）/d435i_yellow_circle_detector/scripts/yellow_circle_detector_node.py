...
class YellowCircleDetectorNode:
    def __init__(self):
        ...
        rospy.loginfo(
            "Yellow circle detector started. target_diameter=%.3f m trusted_depth=[%.2f, %.2f] display_depth=[%.2f, %.2f] close_radius=%.1f px prefer_diameter=%s auto_calibrate=%s calibration_distance=%.3f m calibration_radius=%.1f px camera_info=%s body_transform=%s",
    ...
    def detect_from_binary(self, image, binary_mask):
        ...
        if detection is not None:
            ...
            if self.is_too_close_by_radius(detection["radius"]):
    ...
    def choose_depth(self, raw_depth_m, diameter_depth_m, calibrated_depth_m):
        ...
        if raw_depth_m > 0.0 or diameter_depth_m > 0.0 or calibrated_depth_m > 0.0:
            rospy.logwarn_throttle(
                2.0,
                "All distance estimates are outside trusted range [%.3f, %.3f] m: raw=%.3f diameter=%.3f calibrated=%.3f. Falling back to display range [%.3f, %.3f] m.",
                self.depth_min_valid_m,
                self.depth_max_valid_m,
                raw_depth_m,
                diameter_depth_m,
        ...
        for value, source in fallback_candidates:
            if self.is_displayable_depth(value):
                return value, source
