...
class SemiAutoDropTest:
    def __init__(self):
        ...
        self.setpoint_interval_s = float(
            rospy.get_param("~setpoint_interval_s", 1.0)
        )
    ...
    def wait_for_target_and_confirm(self):
        ...
        while not rospy.is_shutdown():
            ...
            prompt = (
                "检测到桶，按 Enter 后进入 GUIDED，并调整到目标上方 "
                "{:.2f}m 自动瞄准；按 Ctrl+C 取消。"
            ).format(self.target_drop_height_m)
    ...
    def _publish_setpoint(self, aim, dropper):
        ...
        east = getattr(aim, "{}_ned_e".format(prefix))
        down = getattr(aim, "{}_ned_d".format(prefix))
        x, y, z = self._target_enu(aim, dropper)
    ...
    def aim_dropper(self, dropper, reset_loss_guard=True):
        ...
        if self.ground_test:
            ...
            rospy.logwarn("开始 %s 抛投器 GUIDED 瞄准", dropper)
        ...
        while not rospy.is_shutdown():
            ...
            if not self.ground_test:
                ...
                if now >= next_setpoint_at:
                    if not self._publish_setpoint(aim, dropper):
                        return False
            ...
            if not self.ground_test:
                ...
                target_surface_z = target_z - self.target_drop_height_m
                actual_height = current_z - target_surface_z
                height_error = target_z - current_z
                height_aligned = abs(height_error) < self.height_tolerance_m
            aligned = (
