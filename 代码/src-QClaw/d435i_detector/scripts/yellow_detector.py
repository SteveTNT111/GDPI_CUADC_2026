#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
黄色圆检测节点 (ROS Noetic)
- 订阅 /d435i/color_image (彩色图)
- 订阅 /d435i/depth_image (深度图)
- 使用 HSV 颜色过滤 + 轮廓检测识别黄色圆
- 发布检测结果到 /d435i/yellow_circle (自定义消息 YellowCircle)
- 发布标注后的图像到 /d435i/detected_image (sensor_msgs/Image)
"""

import rospy
from sensor_msgs.msg import Image
from cv_bridge import CvBridge
from d435i_detector.msg import YellowCircle
import cv2
import numpy as np


class YellowDetector:
    """黄色圆检测器"""

    def __init__(self):
        # ---- 从参数服务器读取参数 ----
        self.h_min         = rospy.get_param('~h_min', 20)
        self.h_max         = rospy.get_param('~h_max', 40)
        self.s_min         = rospy.get_param('~s_min', 80)
        self.s_max         = rospy.get_param('~s_max', 255)
        self.v_min         = rospy.get_param('~v_min', 80)
        self.v_max         = rospy.get_param('~v_max', 255)
        self.min_area      = rospy.get_param('~min_area', 200)
        self.circ_thresh   = rospy.get_param('~circularity_thresh', 0.6)
        self.depth_scale   = rospy.get_param('~depth_scale', 0.001)
        self.show_window   = rospy.get_param('~show_window', True)

        # ---- ROS 发布者 ----
        self.result_pub = rospy.Publisher('/d435i/yellow_circle', YellowCircle, queue_size=10)
        self.image_pub  = rospy.Publisher('/d435i/detected_image', Image, queue_size=10)

        self.cv_bridge = CvBridge()

        # 缓存最新深度帧
        self.latest_depth = None

        # ---- 订阅者 ----
        self.color_sub = rospy.Subscriber(
            '/d435i/color_image', Image, self.color_callback, queue_size=10)
        self.depth_sub = rospy.Subscriber(
            '/d435i/depth_image', Image, self.depth_callback, queue_size=10)

        rospy.loginfo(
            'Yellow detector started | HSV H:[%d-%d] S:[%d-%d] V:[%d-%d] '
            'min_area:%d circ>%.2f' % (
                self.h_min, self.h_max, self.s_min, self.s_max,
                self.v_min, self.v_max, self.min_area, self.circ_thresh))

    # ------------------------------------------------------------------
    def depth_callback(self, msg):
        """缓存最新深度帧"""
        try:
            self.latest_depth = self.cv_bridge.imgmsg_to_cv2(msg, desired_encoding='16UC1')
        except Exception as e:
            rospy.logwarn('Depth conversion failed: %s' % str(e))

    # ------------------------------------------------------------------
    def color_callback(self, msg):
        """彩色帧回调：HSV 过滤 → 轮廓检测 → 圆度筛选 → 发布"""
        try:
            frame = self.cv_bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        except Exception as e:
            rospy.logwarn('Color conversion failed: %s' % str(e))
            return

        # ---- HSV 黄色过滤 ----
        hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        lower_yellow = np.array([self.h_min, self.s_min, self.v_min])
        upper_yellow = np.array([self.h_max, self.s_max, self.v_max])
        mask = cv2.inRange(hsv, lower_yellow, upper_yellow)

        # 形态学去噪
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
        mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel)
        mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)

        # ---- 轮廓检测 ----
        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        best_cnt = None
        best_score = 0  # 圆度 × 面积 综合评分

        for cnt in contours:
            area = cv2.contourArea(cnt)
            if area < self.min_area:
                continue

            # 圆度 = 4π × 面积 / 周长²，完美圆=1
            perimeter = cv2.arcLength(cnt, True)
            if perimeter == 0:
                continue
            circularity = 4 * np.pi * area / (perimeter * perimeter)

            if circularity < self.circ_thresh:
                continue

            score = circularity * area
            if score > best_score:
                best_score = score
                best_cnt = cnt

        # ---- 发布结果 ----
        result = YellowCircle()
        result.header.stamp = rospy.Time.now()
        result.header.frame_id = 'd435i_color_optical_frame'
        result.x = 0
        result.y = 0
        result.depth = 0.0

        if best_cnt is not None:
            # 最小外接圆
            (cx, cy), radius = cv2.minEnclosingCircle(best_cnt)
            cx, cy, radius = int(cx), int(cy), int(radius)

            result.x = cx
            result.y = cy

            # ---- 取深度 ----
            if self.latest_depth is not None:
                h, w = self.latest_depth.shape[:2]
                if 0 <= cy < h and 0 <= cx < w:
                    raw_depth = self.latest_depth[cy, cx]
                    result.depth = float(raw_depth) * self.depth_scale

            # ---- 在图像上标注 ----
            cv2.drawContours(frame, [best_cnt], -1, (0, 255, 0), 2)
            cv2.circle(frame, (cx, cy), 5, (0, 0, 255), -1)
            cv2.circle(frame, (cx, cy), radius, (0, 255, 0), 2)

            label = '(%d,%d) d=%.2fm' % (cx, cy, result.depth)
            cv2.putText(frame, label, (cx + 10, cy - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

            rospy.loginfo('Yellow circle at (%d,%d), depth=%.3fm' % (cx, cy, result.depth))
        else:
            # 无检测
            cv2.putText(frame, 'No yellow circle detected', (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

        # 发布检测结果消息
        self.result_pub.publish(result)

        # 发布标注后图像
        try:
            annotated_msg = self.cv_bridge.cv2_to_imgmsg(frame, encoding='bgr8')
            annotated_msg.header = msg.header
            self.image_pub.publish(annotated_msg)
        except Exception:
            pass

        # 本地显示（调试用，show_window 参数控制）
        if self.show_window:
            cv2.imshow('Yellow Detector', frame)
            cv2.waitKey(1)


def main():
    rospy.init_node('yellow_detector', anonymous=False)
    node = YellowDetector()
    try:
        rospy.spin()
    except KeyboardInterrupt:
        pass
    finally:
        cv2.destroyAllWindows()


if __name__ == '__main__':
    main()
