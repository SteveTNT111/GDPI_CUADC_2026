...
"""
飞手确认后自动执行：GUIDED -> ARM -> 起飞 2.5 m，并沿启动航向解算两个
WGS84 航点：第一航点默认在 35 m 的投放区中心，第二航点默认在 60 m 的侦察区
中心。最终输入 YES 授权前，飞手必须先输入 RTL（第一航点后返航）或 2
（第一航点 -> 第二航点 -> 返航）；起飞后不再等待终端输入。

安全约束：
  - 程序只允许在飞机确定停在地面时启动；启动后冻结起始航向、位置和目标航点。
  - 解锁后、起飞前重新记录地面的相对 Home 高度，允许该高度不等于 0。
  - 路线选择和两个航点在起飞前冻结；切换模式或解锁前仍须输入授权短语。
...
class Wgs84ForwardRtlNode:
    """执行一/二号 WGS84 前向航点并自动 RTL。"""

    WGS84_A = 6378137.0
    WGS84_F = 1.0 / 298.257223563

    def __init__(self):
        self.takeoff_altitude = float(rospy.get_param("~takeoff_altitude", 2.5))
        self.forward_distance = float(rospy.get_param("~forward_distance", 35.0))
        self.second_forward_distance = float(
            rospy.get_param("~second_forward_distance", 60.0)
        )
        ...
        if self.second_forward_distance <= self.forward_distance:
            raise ValueError("second_forward_distance 必须大于 forward_distance")
        ...
        self.first_target_latitude = None
        self.first_target_longitude = None
        self.second_target_latitude = None
        self.second_target_longitude = None
    ...
    def _freeze_start_and_targets(self):
        ...
        self.first_target_latitude, self.first_target_longitude = (
            self._destination_wgs84(
                self.start_latitude,
        ...
        self.second_target_latitude, self.second_target_longitude = (
            self._destination_wgs84(
                self.start_latitude,
                self.start_longitude,
                self.second_forward_distance,
                self.start_heading_deg,
            )
        )

    @staticmethod
    def _read_terminal_line(prompt):
        try:
    ...
    def _print_route_options(self):
        target_wgs84_altitude = self.start_wgs84_altitude + self.takeoff_altitude
        lines = [
            "",
            "================ 起飞前路线选择 ================",
            "第一航点/投放区中心: {:.2f} m, lat={:.9f}, lon={:.9f}, alt≈{:.3f} m".format(
                self.forward_distance,
                self.first_target_latitude,
                self.first_target_longitude,
                target_wgs84_altitude,
            ),
            "第二航点/侦察区中心: {:.2f} m, lat={:.9f}, lon={:.9f}, alt≈{:.3f} m".format(
                self.second_forward_distance,
                self.second_target_latitude,
                self.second_target_longitude,
                target_wgs84_altitude,
            ),
            "输入 RTL: 起飞 -> 第一航点 -> RTL",
            "输入 2  : 起飞 -> 第一航点 -> 第二航点 -> RTL",
            "其他输入: 取消任务，不切模式、不解锁、不起飞",
            "================================================",
        ]
        print("\n".join(lines), flush=True)

    def _request_route_choice(self):
        response = self._read_terminal_line(
            "请选择飞行路线，输入 'RTL' 或 '2'（输入后仍需 YES 最终授权）: "
        )
        if response is None:
            return False
        choice = response.strip().upper()
        if choice not in ("RTL", "2"):
            rospy.logwarn(
                "路线选择无效：输入=%r，期望='RTL'或'2'；任务取消",
                response.strip(),
            )
            return False
        self.route_choice = choice
        if choice == "RTL":
            rospy.loginfo("路线已冻结：第一航点后自动 RTL")
        else:
            rospy.loginfo("路线已冻结：第一航点 -> 第二航点 -> 自动 RTL")
        return True

    def _print_authorization_summary(self):
        target_wgs84_altitude = self.start_wgs84_altitude + self.takeoff_altitude
        if self.route_choice == "RTL":
            selected_plan = (
                "自动解锁 -> 起飞 {:.2f} m -> 第一航点 {:.2f} m "
                "-> 悬停 {:.1f} s -> RTL"
            ).format(
        ...
        lines = [
            ...
            "第一航点     : lat={:.9f}, lon={:.9f}, alt≈{:.3f} m".format(
                self.first_target_latitude,
                self.first_target_longitude,
                target_wgs84_altitude,
            ),
            "第二航点     : lat={:.9f}, lon={:.9f}, alt≈{:.3f} m".format(
                self.second_target_latitude,
                self.second_target_longitude,
                target_wgs84_altitude,
            ),
            "已选路线     : {}".format(selected_plan),
            "注意：解锁后将重新记录地面 rel_alt，目标命令高度=地面 rel_alt+{:.2f} m。".format(
                self.takeoff_altitude
            ),
            "      两个航点和路线已冻结；YES 后不再等待终端输入。",
    ...
    def _request_authorization(self):
        ...
        response = self._read_terminal_line(prompt)
        if response is None:
            return False
    ...
    def _make_global_target(self, latitude, longitude):
    ...
    def hold_global_target(self, target, hover_time):
        indefinite_hover = hover_time is None
        if indefinite_hover:
            rospy.logwarn("已抵达目标，将持续发送 setpoint 悬停，不自动进入 RTL")
        else:
            rospy.loginfo("已抵达目标，持续发送 setpoint 悬停 %.1f s", hover_time)
        held_time = 0.0
        last_loop_at = time.monotonic()
        connection_lost_since = None
        rate = rospy.Rate(self.setpoint_rate)
        while not rospy.is_shutdown() and (
            indefinite_hover or held_time < hover_time
        ):
            now = time.monotonic()
    ...
    def run(self):
        ...
        try:
            self._freeze_start_and_targets()
        except ValueError as exc:
            rospy.logerr("起飞前检查失败: %s", exc)
            return 1
        self._print_route_options()
        if not self._request_route_choice():
            return 2
        ...
        first_target = self._make_global_target(
            self.first_target_latitude, self.first_target_longitude
        )
        rospy.loginfo(
            "开始飞往第一航点（投放区中心，前向 %.2f m）", self.forward_distance
        )
        if not self.fly_to_target(first_target):
            self._abort_to_rtl_or_land()
            return 1
        if not self.hold_global_target(first_target, self.waypoint_hover_time):
            self._abort_to_rtl_or_land()
            return 1

        if self.route_choice == "2":
            second_target = self._make_global_target(
                self.second_target_latitude, self.second_target_longitude
            )
            rospy.loginfo(
                "按起飞前选择继续飞往第二航点（侦察区中心，前向 %.2f m）",
                self.second_forward_distance,
            )
            if not self.fly_to_target(second_target):
                self._abort_to_rtl_or_land()
                return 1
            if not self.hold_global_target(
                second_target, self.waypoint_hover_time
            ):
                self._abort_to_rtl_or_land()
                return 1

        rospy.loginfo("所选航点流程完成，自动请求 RTL")
