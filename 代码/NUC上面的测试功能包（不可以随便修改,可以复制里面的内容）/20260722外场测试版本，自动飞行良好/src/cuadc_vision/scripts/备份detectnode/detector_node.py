...
class DetectorNode:
    ...
    def __init__(self):
        ...
        # 抛投器相对相机光心在机体前向 X 上的安装偏移。
        # 相机光学坐标 Y+ 向图像下方，且 Y_cam -> -X_body，
        # 因此机体前方 +5 cm 的 A 抛投器对应 camera_y=-0.05 m。
        self.dropper_a_forward_offset_m = float(
            rospy.get_param("~dropper_a_forward_offset_m", 0.05)
        )
        self.dropper_b_forward_offset_m = float(
            rospy.get_param("~dropper_b_forward_offset_m", -0.05)
        )
        ...
        rospy.loginfo(
            "Dropper crosshairs: A body_x=%+.3fm (camera_y=%+.3fm), "
            "B body_x=%+.3fm (camera_y=%+.3fm)",
            self.dropper_a_forward_offset_m,
            -self.dropper_a_forward_offset_m,
            self.dropper_b_forward_offset_m,
            -self.dropper_b_forward_offset_m,
        )
        if self.publish_geo_target:
    ...
    def image_callback(self, msg):
        ...
        # 根据目标深度，把 A/B 抛投器的实际前后安装偏移投影为图像准星。
        aim_offsets = self.compute_dropper_aim_offsets(result_msg)
        if aim_offsets["valid"]:
            rospy.loginfo_throttle(
                1.0,
                "AIM OFFSET px | A dx=%+.1f dy=%+.1f | B dx=%+.1f dy=%+.1f",
                aim_offsets["a_dx"], aim_offsets["a_dy"],
                aim_offsets["b_dx"], aim_offsets["b_dy"],
            )
        ...
        self.draw_dropper_crosshairs(annotated, aim_offsets)

        # 4c. 画最佳目标的详细标注（见 draw_overlay）
        if best is not None:
            self.draw_overlay(annotated, best, aim_offsets)
        ...
        if best is not None:
            ...
            binfo.delta_x = 0.0
            binfo.delta_y = 0.0
        binfo.aim_offsets_valid = bool(aim_offsets["valid"])
        binfo.a_crosshair_x = float(aim_offsets["a_x"])
        binfo.a_crosshair_y = float(aim_offsets["a_y"])
        binfo.a_delta_x = float(aim_offsets["a_dx"])
        binfo.a_delta_y = float(aim_offsets["a_dy"])
        binfo.b_crosshair_x = float(aim_offsets["b_x"])
        binfo.b_crosshair_y = float(aim_offsets["b_y"])
        binfo.b_delta_x = float(aim_offsets["b_dx"])
        binfo.b_delta_y = float(aim_offsets["b_dy"])
    ...
    def compute_dropper_aim_offsets(self, det):
        """计算目标框中心相对 A/B 抛投准星的像素差。

        A/B 均位于相机纵向中心线上，只存在机体前后方向安装偏移。
        机体 X 与相机 Y 的关系为 X_body=-Y_cam，所以准星相机坐标为：
            camera_y = -dropper_forward_offset_m
        再通过 v = cy + fy * camera_y / depth 投影到图像。
        """
        result = {
            "valid": False,
            "a_x": 0.0, "a_y": 0.0, "a_dx": 0.0, "a_dy": 0.0,
            "b_x": 0.0, "b_y": 0.0, "b_dx": 0.0, "b_dy": 0.0,
        }
        if det is None or not det.detected or not det.position_valid:
            return result
        depth_m = float(det.camera_z_m)
        ci = self.get_camera_info()
        if ci is None or depth_m <= 0.0 or not math.isfinite(depth_m):
            return result
        fx, fy = float(ci.K[0]), float(ci.K[4])
        cx, cy = float(ci.K[2]), float(ci.K[5])
        if fx <= 0.0 or fy <= 0.0:
        ...
        a_camera_y_m = -self.dropper_a_forward_offset_m
        b_camera_y_m = -self.dropper_b_forward_offset_m
        a_x = cx
        a_y = cy + fy * a_camera_y_m / depth_m
        b_x = cx
        b_y = cy + fy * b_camera_y_m / depth_m

        result.update({
            "valid": True,
            "a_x": a_x,
            "a_y": a_y,
            "a_dx": float(det.center_x) - a_x,
            "a_dy": float(det.center_y) - a_y,
            "b_x": b_x,
            "b_y": b_y,
            "b_dx": float(det.center_x) - b_x,
            "b_dy": float(det.center_y) - b_y,
        })
        return result
    ...
    def draw_overlay(self, image, det, aim_offsets=None):
        """
        ...
        if det.position_valid:
            ...
            if aim_offsets and aim_offsets["valid"]:
                # 第 4/5 行：识别框中心相对 A/B 物理准星的像素差。
                self.draw_text_bg(
                    image,
                    "A dx={:+.0f} dy={:+.0f}px".format(
                        aim_offsets["a_dx"], aim_offsets["a_dy"]
                    ),
                    bx, by + line_h * 3,
                    font_scale=font_scale, text_color=(255, 0, 255),
                )
                self.draw_text_bg(
                    image,
                    "B dx={:+.0f} dy={:+.0f}px".format(
                        aim_offsets["b_dx"], aim_offsets["b_dy"]
                    ),
                    bx, by + line_h * 4,
                    font_scale=font_scale, text_color=(255, 255, 0),
                )
        else:
            self.draw_text_bg(
                image, "depth none",
    ...
    def draw_dropper_crosshairs(self, image, aim_offsets, size=14):
        """绘制 A（紫色）和 B（青色）抛投准星。"""
        if not aim_offsets or not aim_offsets["valid"]:
            return
        h, w = image.shape[:2]
        for label, x_key, y_key, color in (
            ("A", "a_x", "a_y", (255, 0, 255)),
            ("B", "b_x", "b_y", (255, 255, 0)),
        ):
            x = int(round(aim_offsets[x_key]))
            y = int(round(aim_offsets[y_key]))
            if x < 0 or x >= w or y < 0 or y >= h:
                continue
            cv2.line(image, (max(0, x - size), y), (min(w - 1, x + size), y), color, 2)
            cv2.line(image, (x, max(0, y - size)), (x, min(h - 1, y + size)), color, 2)
            # A/B 抛投器预计中心点：在相机 Y 轴上分别对应 -5 cm / +5 cm。
            # 使用实心点明确标出中心，同时保留十字线便于人工对准。
            cv2.circle(image, (x, y), 5, color, -1)
            cv2.circle(image, (x, y), 8, color, 1)
            cv2.putText(
                image, label, (min(w - 16, x + 7), max(15, y - 7)),
