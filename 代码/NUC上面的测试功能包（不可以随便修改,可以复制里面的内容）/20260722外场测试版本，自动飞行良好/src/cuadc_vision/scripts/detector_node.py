...
功能：
  ...
  5. 读取 MAVROS 飞控数据（连接状态、电压、电流、已用电量、卫星数、RTK GPS、Z 高度）
...
class DetectorNode:
    ...
    def __init__(self):
        ...
        self.night_mode = get_bool_param("~night_mode", False)
        self.night_conf_threshold = float(
            rospy.get_param("~night_conf_threshold", 0.50)
        )
        self.night_gamma = float(rospy.get_param("~night_gamma", 0.75))
        self.night_clahe_clip_limit = float(
            rospy.get_param("~night_clahe_clip_limit", 2.0)
        )
        self.night_clahe_grid_size = int(
            rospy.get_param("~night_clahe_grid_size", 8)
        )
        if self.night_mode and 0.0 < self.night_conf_threshold <= 1.0:
            self.conf_threshold = self.night_conf_threshold
        if not 0.0 < self.night_gamma:
            rospy.logwarn("Invalid ~night_gamma=%s; using 0.75", self.night_gamma)
            self.night_gamma = 0.75
        if self.night_clahe_clip_limit <= 0.0:
            rospy.logwarn(
                "Invalid ~night_clahe_clip_limit=%s; using 2.0",
                self.night_clahe_clip_limit,
            )
            self.night_clahe_clip_limit = 2.0
        self.night_clahe_grid_size = max(2, self.night_clahe_grid_size)
        gamma_values = np.arange(256, dtype=np.float32) / 255.0
        self._night_gamma_lut = np.clip(
            np.power(gamma_values, self.night_gamma) * 255.0, 0.0, 255.0
        ).astype(np.uint8)
        self._night_clahe = cv2.createCLAHE(
            clipLimit=self.night_clahe_clip_limit,
            tileGridSize=(self.night_clahe_grid_size, self.night_clahe_grid_size),
        )
        ...
        self.fc_used_mah = None           # MAVLink BATTERY_STATUS 已消耗电量 (mAh)
        ...
        if self.night_mode:
            rospy.logwarn(
                "Detector NIGHT MODE enabled: conf=%.2f gamma=%.2f "
                "CLAHE clip=%.2f grid=%d",
                self.conf_threshold,
                self.night_gamma,
                self.night_clahe_clip_limit,
                self.night_clahe_grid_size,
            )
    ...
    def fc_battery_cb(self, msg):
        """电池状态回调"""
        charge_ah = getattr(msg, 'charge', float('nan'))
        used_mah = None
        if math.isfinite(charge_ah) and charge_ah >= 0.0:
            # MAVROS 将 MAVLink BATTERY_STATUS.current_consumed 从 mAh
            # 换算为 BatteryState.charge 的 Ah；显示时换回飞控上报的 mAh。
            used_mah = charge_ah * 1000.0
        with self.fc_lock:
            self.fc_voltage = getattr(msg, 'voltage', 0.0)
            self.fc_current = getattr(msg, 'current', 0.0)
            self.fc_used_mah = used_mah
    ...
    def _preprocess_for_inference(self, frame):
        """夜间模式增强模型输入；白天模式直接返回原图。"""
        if not self.night_mode:
            return frame

        # CLAHE 只增强 LAB 亮度通道，尽量不改变桶的综合色彩特征。
        lab = cv2.cvtColor(frame, cv2.COLOR_BGR2LAB)
        lightness, channel_a, channel_b = cv2.split(lab)
        lightness = self._night_clahe.apply(lightness)
        enhanced = cv2.cvtColor(
            cv2.merge((lightness, channel_a, channel_b)), cv2.COLOR_LAB2BGR
        )
        # gamma < 1 提亮暗部；固定查找表避免每帧进行浮点幂运算。
        return cv2.LUT(enhanced, self._night_gamma_lut)
    ...
    def image_callback(self, msg):
        ...
        frame = self._preprocess_for_inference(frame)
    ...
    def _build_bottom_bar(self, now, n_detections):
        ...
        # 左栏的 Z 始终来自 D435 当前深度图中心区域的中值，作为相机直接
        # 测得的离地距离。不要在飞控 local Z 与相机深度之间自动切换。
        # 传入 0, 0 让函数按实际深度图尺寸取几何中心，避免依赖默认分辨率。
        center_depth_z = self.get_center_depth(0, 0)
        if center_depth_z > 0.01 and math.isfinite(center_depth_z):
            center_depth_text = "D435 CENTER Z: {:.2f} m".format(center_depth_z)
        else:
            center_depth_text = "D435 CENTER Z: ---"

        if not self._has_fc_data or not connected:
            left.append((
                "FC: disconnected  |  {}".format(center_depth_text),
                self._CLR_WARN,
            ))
            ...
            if voltage > 0.0:
                used_mah_text = (
                    "{:.0f}mAh".format(used_mah)
                    if used_mah is not None else "N/A"
                )
                left.append((
                    "Bat: {:.1f}V  {:.1f}A  Used: {}  Sat: {}  {}  |  AMMO: A-{} B-{}".format(
                        voltage, abs(current), used_mah_text,
