...
│   ├── BucketInfo.msg                         #   目标数量 + 旧版像素偏差接口
│   └── BucketAimInfo.msg                      #   A/B 抛投瞄准点、米制偏差和 NED
├── models/
├── CMakeLists.txt
└── package.xml
```
...
| **geographiclib** | 大地测量——WGS84 椭球面上的正反算、相机→机体→ENU→经纬高的坐标变换（已内联到 `detector_node.py`） | [官方文档](https://geographiclib.sourceforge.io/Python/doc/) · [PyPI](https://pypi.org/project/geographiclib/) · [GitHub](https://github.com/geographiclib/geographiclib-python) |
...
```bash
chmod +x ~/catkin_ws/src/cuadc_vision/scripts/*.py
```
...
> 理解这些坐标系的定义和它们之间的变换关系，是读懂 `detector_node.py` 内联坐标变换逻辑的前提。
...
> **所在节点：** `detector_node.py` → `transform_camera_to_body()`
...
> 如果实际安装方向不同，需要同步校准 `detector_node.py` 的相机→机体安装关系和 launch 中的静态 TF 参数。
...
> **所在节点：** `detector_node.py` → `quaternion_rotate_vector()`
...
> **所在节点：** `detector_node.py` → `enu_offset_to_geodetic()`
...
│  detector_node    detector_node     detector_node   detector_node      │
│                                                                       │
│  ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐ ┌─────┐│
...
| 2 | 相机 → 机体 | (X_cam, Y_cam, Z_cam) | (X_body, Y_body, Z_body) | `detector_node.py:transform_camera_to_body()` | 相机安装关系 |
| 3 | 机体 → ENU | (X_body, Y_body, Z_body) | (E, N, U) | `detector_node.py:quaternion_rotate_vector()` | MAVROS 四元数 |
| 4 | ENU → WGS84 | (E, N, U) + 无人机 GPS | (lat, lon, alt) | `detector_node.py:enu_offset_to_geodetic()` | MAVROS GPS + geographiclib |

#### GeoTarget 消息承载所有中间结果

`detector_node` 可选输出的 `GeoTarget` 消息包含了变换链上**每一步**的坐标值，方便调试：
...
| 大地坐标目标 | 全链路输出 | `/vision/target_global` | `detector_node`（可选） |
...
#### 实飞标定：相机视野、搜索高度与投放区覆盖（2026-07-20）

以下数据来自白天现场实测，用于后续修改半自动投放测试脚本和自动搜索脚本：

- D435i 相机光心离地约 `2.4m` 时，画面覆盖地面前后长度约 `1.5m`，方向与飞机
  前进方向一致。
- 现场同时记录横向约 `4.5` 块、每块边长 `0.5m` 的地砖，并口述横向覆盖约
  `2.5m`。但 `4.5 × 0.5 = 2.25m`，两项记录存在 `0.25m` 差异；重新测量确认前，
  自动搜索航线按较保守的 `2.25m` 横向有效视野规划，避免漏扫。
- 投放区尺寸为横向宽 `8m`、纵向长 `5m`；其中 `5m` 长度方向就是飞机的前进方向。
- 白天实测相机离地高度在 `1.0~2.7m` 之间均可得到较稳定的目标跟踪。

若地面近似水平、相机保持垂直向下且图像裁剪不变，可先把视野尺寸近似看作与
相机离地高度成正比。以口述的 `2.5m × 1.5m @ 2.4m` 为基准：

| 相机离地高度 | 估算横向视野 | 估算前进方向视野 |
|---:|---:|---:|
| `2.7m` | `2.81m` | `1.69m` |
| `2.4m` | `2.50m` | `1.50m` |
| `2.0m` | `2.08m` | `1.25m` |
...
搜索进入投放区时，推荐控制目标是 **D435 CENTER Z / 相机实际离地距离约
`2.4m`**：该高度已有覆盖范围实测值，处在稳定跟踪区间内，并给 `2.7m` 的识别
上限保留约 `0.3m` 裕量。WGS84 脚本中的 `takeoff_altitude` 是飞控相对 Home 高度，
不是相机离地距离；应结合相机光心低于飞控机体原点的安装高度、地面坡度和气压计/
EKF 偏差换算。当前尚无相机高度闭环时，可先以 `takeoff_altitude:=2.5` 做低速实飞
标定，并以画面中的 `D435 CENTER Z` 是否稳定在约 `2.4m` 为准修正，不能仅靠增加
WGS84 高度来掩盖持续掉高问题。

对于 `8m × 5m` 投放区，若检测节点在航段中持续处理视频，建议采用 **4 条沿
5m 长度方向的蛇形航线，共 8 个端点航点**。按保守横向有效视野 `2.25m` 计算，
以投放区一角为 `(横向 x=0, 前进 y=0)`，可先使用以下局部规划位置：

```text
(1.125, 0.75) -> (1.125, 4.25)
               -> (3.042, 4.25) -> (3.042, 0.75)
               -> (4.958, 0.75) -> (4.958, 4.25)
               -> (6.875, 4.25) -> (6.875, 0.75)
```

这 8 个点是连续蛇形扫描的航段端点，不是只在航点悬停拍一张图。只有在 RTK 已固定、
航线横向误差能稳定控制在约 `0.2m` 内时，才把 4 条航线/8 个端点作为默认方案。
如果横向视野最终确认只有 `2.25m`、RTK 未固定，或横向跟踪误差可能超过 `0.2m`，
应改用 **5 条航线/10 个端点** 的保守方案；5 条航线中心可取横向
`x = 1.125, 2.563, 4.000, 5.438, 6.875m`，纵向端点仍取 `y = 0.75/4.25m`。
如果自动搜索只在
航点处观察、不在航段中连续识别，则理论上至少需要 `4 × 4 = 16` 个观察点，且还应
额外留重叠量；因此正式自动搜索应优先采用连续视频扫描：RTK/跟踪稳定时使用 8 个
端点，定位或视野不确定时使用 10 个端点。实际生成
WGS84 航点前，必须把局部横向/前进坐标按照投放区朝向旋转到 N/E，并用现场边界点
校验，不能把“飞机启动航向”直接假定为投放区的 5m 长度方向。

当前 `cuadc_control/one_key_takeoff_wgs84_forward_rtl.py` 已支持沿启动航向解算
`35m` 投放区中心和 `60m` 侦察区中心两个前向 WGS84 航点，并在最终 `YES` 前通过
`RTL/2` 选择第一航点返航或第二航点后返航；默认 `takeoff_altitude=2.5m`。它仍不是
上述 V/之字形区域扫描任务。实飞出现“前飞过程中
越来越低”时，应记录 `/mavros/global_position/rel_alt`、`D435 CENTER Z`、飞行模式、
目标高度和垂直速度，先判断是相对 Home 高度估计漂移、真实高度控制偏差、地面起伏，
还是相机深度变化，再决定修改控制器。
...
**功能：** 一键启动 D435i 相机 + YOLO 检测。支持**自动启动 roscore / MAVROS**（如未运行），读取飞控数据并在画面底部半透明两栏信息栏显示（左=状态，右=坐标）。加载 YOLOv8 模型逐帧推理，保留最近 5 帧桶中心平均稳定输出，并计算 A/B 两个真实抛投瞄准点。显式传入 `night_mode:=true` 时，同时启用 D435i 夜间曝光参数和检测输入图像增强；无附加参数时仍保持原白天模式。
...
- 用 `rostopic echo /vision/bucket/aim_info` 查看 A/B 像素点、米制偏差和 NED
...
```bash
python3 ~/catkin_ws/src/cuadc_vision/scripts/detector_node.py
...
```bash
roslaunch cuadc_vision detector_node.launch show_window:=true aim_offset_m:=0.05
```
...
`night_mode:=true` 会同时执行以下处理：

- D435i RGB 固定曝光 `10000 us`、增益 `64`、白平衡 `4500 K`，并选择中国场地使用的 `50 Hz` 防频闪。固定参数可减少灯光下自动曝光来回调整造成的画面明暗抽动。
- YOLO 推理前对 LAB 亮度通道执行 CLAHE 局部对比度增强，再用 `gamma=0.75` 提亮暗部；色度通道不做自适应拉伸，降低灯光色偏被进一步放大的风险。
- 夜间检测阈值默认从白天 launch 的 `0.60` 调整为 `0.50`。这只是小幅放宽，仍保留后续多帧稳定、NED 跳变拒绝和最终速度/位置门限，不建议为了“框一直存在”直接降到很低。

夜间主要参数：

| 参数 | 夜间默认值 | 说明 |
|------|-----------:|------|
| `night_mode` | `false` | 只有显式设为 `true` 才启用夜间处理 |
| `night_exposure_us` | `10000` | RGB 固定曝光；设为 `0` 或负数时改用自动曝光 |
| `night_gain` | `64` | 固定曝光模式下的 RGB 增益 |
| `night_white_balance` | `4500` | 固定白平衡；设为 `0` 或负数时改用自动白平衡 |
| `night_power_line_frequency` | `1` | RealSense 选项：`1=50 Hz`，`2=60 Hz` |
| `night_conf_threshold` | `0.50` | 夜间 YOLO 最低置信度 |
| `night_gamma` | `0.75` | 小于 `1` 提亮暗部；过小会放大噪声 |
| `night_clahe_clip_limit` | `2.0` | 局部对比度增强强度 |
| `night_clahe_grid_size` | `8` | CLAHE 网格尺寸 |

如果场地灯光明暗变化很大，可先改用自动曝光，但保持固定帧率优先级：

```bash
roslaunch cuadc_vision detector_node.launch \
  night_mode:=true \
  night_exposure_us:=0 \
  night_white_balance:=0 \
  show_window:=true
```

如果固定曝光下画面过曝，先降低曝光，再降低增益；如果画面仍偏暗，则按相反方向调整。例如：

```bash
roslaunch cuadc_vision detector_node.launch \
  night_mode:=true \
  night_exposure_us:=6000 \
  night_gain:=48 \
  night_conf_threshold:=0.55 \
  show_window:=true
```
...
```bash
roslaunch cuadc_vision camera_node.launch night_mode:=true
```

#### 识别框跳动与最后 NED 的处理

识别节点仍遵守“当前帧无有效检测就发布无效消息”的原则，不会把旧框、旧像素 Delta 伪装成当前有效检测。原因是旧的像素偏差不能证明飞机此刻仍满足最终投放门限，直接复用可能造成误投。

在 `cuadc_control` 的 `semi_auto_drop_test.py` 和 `auto_drop_test.py` 中，已经存在更安全的最后坐标处理：

```text
有效视觉帧 -> 解算绝对目标 NED -> 低通滤波 + NED 跳变拒绝 -> 保存 locked_ned
短时置信度掉落/识别框消失 -> 保持当前飞机位置，不清除 locked_ned
持续丢失 -> 以最后 locked_ned 为中心升高并局部搜索
重新识别稳定 -> 更新 locked_ned；重新满足位置、高度和速度稳定门限后才允许投放
```

因此夜间的单帧或短时跳框不会立即把瞄准中心带走，也不会使用旧像素误判“已对准”。若只运行 `detector_node.launch`，节点只负责发布检测和 NED，不会自行控制飞机；需要最后 NED 保持/重捕获时，应配合上述 `cuadc_control` 瞄准脚本使用。

夜间测试建议先在地面记录 30～60 秒：同时观察窗口、`/vision/bucket/aim_info` 的 `confidence` 和 NED，并分别测试目标在画面中心及边缘时是否稳定。若置信度稳定但 NED 跳变，优先检查深度图和本地位姿，不应继续降低 YOLO 阈值。
...
| `/vision/bucket/aim_info` | BucketAimInfo | 稳定桶中心、A/B 瞄准点、像素/米制 Delta 和 NED |
...
| `aim_offsets_valid` | bool | 兼容字段；与新消息的 `valid` 同步 |
| `a_crosshair_x/y` | float32 | 兼容字段；A 瞄准点像素坐标 |
| `b_crosshair_x/y` | float32 | 兼容字段；B 瞄准点像素坐标 |
| `a_delta_x/y` | float32 | 兼容字段；A 点相对画面中心的像素 Delta |
| `b_delta_x/y` | float32 | 兼容字段；B 点相对画面中心的像素 Delta |
...
#### `/vision/bucket/aim_info` 消息详解 (BucketAimInfo)

桶识别框中心先沿用现有的 5 帧算术平均稳定逻辑；连续 10 帧没有目标时清空旧缓冲。识别到桶且深度、相机内参有效后：

- 桶中心画为**红色点**。
- A 抛投器位于相机前方；使用 A 时飞机需要向后瞄，因此 A 瞄准点位于桶中心下方 `aim_offset_m`，画为带 `A` 标签的**蓝色点**。
- B 抛投器位于相机后方；使用 B 时飞机需要向前瞄，因此 B 瞄准点位于桶中心上方 `aim_offset_m`，画为带 `B` 标签的**蓝色点**。
- 默认 `aim_offset_m=0.05 m`，不是固定像素值。

像素偏移通过当前深度和相机纵向焦距动态计算：

```text
offset_y_px = aim_offset_m × fy / depth_m
A = (bucket_center_x, bucket_center_y + offset_y_px)  # 图像下方，y 增大
B = (bucket_center_x, bucket_center_y - offset_y_px)  # 图像上方，y 减小
```
...
- `*_delta_*_px`：像素单位，图像 x 向右、y 向下为正。
- `*_delta_*_m`：使用当前目标深度和相机内参估算的米制偏差。
- 半自动投放脚本可用 `abs(delta_x_m) < 0.15` 且 `abs(delta_y_m) < 0.15` 判断是否瞄准。
- 所有 `a_delta_*` 和 `a_ned_*` 均对应桶下方的 A 点；所有 `b_delta_*` 和 `b_ned_*` 均对应桶上方的 B 点。

| 字段 | 类型 | 含义 |
|------|------|------|
| `header` | Header | 图像时间戳和 frame_id |
| `count` | int32 | 当前帧通过类别过滤的桶数量 |
| `valid` | bool | 桶、深度、内参、A/B 图像点和米制 Delta 是否有效 |
| `bucket_center_x/y` | int32 | 平滑后的桶识别框中心像素 |
| `a_aim_x/y` | int32 | A 抛投器真实瞄准点像素 |
| `b_aim_x/y` | int32 | B 抛投器真实瞄准点像素 |
| `a_delta_x/y_px` | float32 | A 点相对画面中心的像素 Delta |
| `b_delta_x/y_px` | float32 | B 点相对画面中心的像素 Delta |
| `a_delta_x/y_m` | float32 | A 点相对画面中心的米制 Delta |
| `b_delta_x/y_m` | float32 | B 点相对画面中心的米制 Delta |
| `a_ned_valid` | bool | A 点本地绝对 NED 是否有效 |
| `a_ned_n/e/d` | float32 | A 点在 MAVROS 本地 NED 下的 N/E/D |
| `b_ned_valid` | bool | B 点本地绝对 NED 是否有效 |
| `b_ned_n/e/d` | float32 | B 点在 MAVROS 本地 NED 下的 N/E/D |
| `confidence` | float32 | 最佳桶检测置信度 |
| `source_class` | string | YOLO 输出的目标类别名 |

有效性规则：

| 条件 | `valid` | `a_ned_valid/b_ned_valid` |
|------|---------|---------------------------|
| 无桶，或深度/相机内参无效 | false | false |
| 视觉数据有效，但没有 `local_pose` | true | false |
| 视觉数据和合法 `local_pose` 都有效 | true | true |

NED 计算复用 detector_node 已有的相机光学系 → 机体系 FRD → ENU → NED 逻辑。GPS 经纬度不是本地 NED 的必要条件；没有 GPS 时只影响 WGS84 结果。无飞控位姿时节点仍持续发布消息，不会崩溃。

```bash
# 终端 1：启动相机和检测节点
source ~/catkin_ws/devel/setup.bash
roslaunch cuadc_vision detector_node.launch aim_offset_m:=0.05
...
当前实现由 `detector_node` 内部的 `_camera_position_to_body()` 直接复用安装映射完成，相机→机体不依赖实时 TF 查询。`project_pixel()` 的 `invert_camera_x` 默认是 `true`，因此更换相机朝向后必须结合实机标定检查 X 方向符号。
...
| 抛投中 | **B DROP!!!** | 红色 | 常亮 3 秒 | main.py 发送 B 抛投器事件 |
...
- 最佳目标 `best` 在 detector_node 内部完成相机安装映射 + 四元数旋转
- 结果显示在底部栏**右栏**，并供 `BucketAimInfo` 的 A/B NED 字段复用
...
| `aim_offset_m` | 0.05 | 瞄准点相对桶中心的真实距离；A 在下方、B 在上方 (m) |
...
### 5. 坐标变换 — 已内联到 detector_node.py

**启动文件：** `detector_node.launch`

**当前定位：** 当前 NUC 工作空间中没有独立的 `geopose_node.py`；相关安装项也已从 CMake 清理。相机→机体→ENU/NED/WGS84 变换统一由 `detector_node.py` 完成。

内联坐标变换主要用于：

- 在标注画面底部显示桶的 BODY、NED 和 WGS84 坐标
- 为 `/vision/bucket/aim_info` 生成 A/B 的本地绝对 NED
- `publish_geo_target:=true` 时发布 `/vision/target_global`
...
**变换流程（代码实现）：**
```
1. transform_camera_to_body() camera_optical_frame → base_link (机体 FRD)
...
> **开启 GeoTarget 发布：** detector_node 默认只缓存和显示变换结果；需要话题时开启：
>
> ```bash
> roslaunch cuadc_vision detector_node.launch publish_geo_target:=true
> ```
>
> 当前工作空间无需、也没有单独的 geopose 节点。
...
<details>
<summary><b>📐 gptimage 绘图提示 6：detector_node 内联变换链代码流程图</b></summary>

```
A flowchart-style diagram showing the detector_node internal coordinate-transform pipeline:
...
Label at top: "detector_node.py 内联变换流程"
...
# ③ 直接 Python 启动（全自动）
python3 ~/catkin_ws/src/cuadc_vision/scripts/one_key_takeoff.py
```
...
| detector_node | `/vision/bucket/aim_info` | A/B 瞄准点、像素/米制 Delta、NED |
| main.py | `/vision/mission_status` | 弹药/瞄准/抛投事件 |
| detector_node（可选） | `/vision/target_global` | 大地坐标（`publish_geo_target=true`） |
...
**夜间模式画面过曝、偏暗或仍有灯光条纹：** 固定曝光默认 `10000 us`，先用 `night_exposure_us:=6000` 等值逐步降低；偏暗时再提高曝光或 `night_gain`。中国 50 Hz 市电环境保持 `night_power_line_frequency:=1`。如果灯光亮度持续变化，可设 `night_exposure_us:=0` 启用自动曝光，但应重新检查检测 FPS 和置信度是否稳定。
...
**坐标与画面不一致：** 确认你理解相机光学坐标系的定义：X+ = 右，Y+ = 下，Z+ = 光轴前方。窗口画面中心的坐标轴（红 x→ 右、绿 y↓ 下）标注了正方向。再检查 `invert_camera_x`、`camera_mount_z` 和相机实际安装方向；相机→机体映射由 detector_node 内部完成。
...
```bash
# 检查 launch 文件是否存在
ls ~/catkin_ws/src/cuadc_vision/launch/
...
# 应输出：/home/lab/catkin_ws/src/cuadc_vision
```
