/*代码功能，全流程最终版本2，决定采用经纬度发点方式新路径。瞄准阶段采用Local发点
是否仿真验证：
是否实飞验证：
...
int main(int argc, char **argv) {
    ...
    nh.getParam("position4_lat", map[4].lat);
    nh.getParam("position4_lon", map[4].lon);
    nh.getParam("position5_lat", map[5].lat);
    nh.getParam("position5_lon", map[5].lon);
    nh.getParam("position6_lat", map[6].lat);
    nh.getParam("position6_lon", map[6].lon);
    nh.getParam("position7_lat", map[7].lat);
    nh.getParam("position7_lon", map[7].lon);
    nh.getParam("position8_lat", map[8].lat);
    nh.getParam("position8_lon", map[8].lon);
    nh.getParam("position9_lat", map[9].lat);
    nh.getParam("position9_lon", map[9].lon);
    nh.getParam("position10_lat", map[10].lat);
    nh.getParam("position10_lon", map[10].lon);
    nh.getParam("position11_lat", map[11].lat);
    nh.getParam("position11_lon", map[11].lon);
    nh.getParam("position12_lat", map[12].lat);
    nh.getParam("position12_lon", map[12].lon);
    nh.getParam("position13_lat", map[13].lat);
    nh.getParam("position13_lon", map[13].lon);
    nh.getParam("position14_lat", map[14].lat);
    nh.getParam("position14_lon", map[14].lon);
    nh.getParam("position15_lat", map[15].lat);
    nh.getParam("position15_lon", map[15].lon);
    nh.getParam("position16_lat", map[16].lat);
    nh.getParam("position16_lon", map[16].lon);
    nh.getParam("position17_lat", map[17].lat);
    nh.getParam("position17_lon", map[17].lon);
    nh.getParam("position18_lat", map[18].lat);
    nh.getParam("position18_lon", map[18].lon);
    nh.getParam("position19_lat", map[19].lat);
    nh.getParam("position19_lon", map[19].lon);
    nh.getParam("position20_lat", map[20].lat);
    nh.getParam("position20_lon", map[20].lon);
    nh.getParam("position21_lat", map[21].lat);
    nh.getParam("position21_lon", map[21].lon);
    nh.getParam("position22_lat", map[22].lat);
    nh.getParam("position22_lon", map[22].lon);
    nh.getParam("position23_lat", map[23].lat);
    nh.getParam("position23_lon", map[23].lon);
    nh.getParam("position24_lat", map[24].lat);
    nh.getParam("position24_lon", map[24].lon);
    nh.getParam("position25_lat", map[25].lat);
    nh.getParam("position25_lon", map[25].lon);
    nh.getParam("position26_lat", map[26].lat);
    nh.getParam("position26_lon", map[26].lon);
    nh.getParam("position27_lat", map[27].lat);
    nh.getParam("position27_lon", map[27].lon);
    nh.getParam("position28_lat", map[28].lat);
    nh.getParam("position28_lon", map[28].lon);
    nh.getParam("position29_lat", map[29].lat);
    nh.getParam("position29_lon", map[29].lon);
    ...
    while (ros::ok()) {
                        ...
                        if (arming_client.call(arm_cmd)) {
                            if (arm_cmd.response.success) {
                                ...
                                if (world_forward.norm() > 1e-3) { // 避免除以零
                                    ...
                                    Eigen::Vector3d euler = start_q_eigen.toRotationMatrix().eulerAngles(2, 1, 0);
                                    yaw = euler(0);
        ...
        case TAR_FIND:
        {
            ...
            if (global_pos_check_without_z(map[wp_index].lat, map[wp_index].lon, err_without_z) && 
                    altitude_check(det_height, err_z_takeoff))
            {
                // 切换到下一个目标点
                ...
                //go_to_pub();
                go_to_global_pub(map[wp_index].lat, map[wp_index].lon, det_height);
                wp_index++;
