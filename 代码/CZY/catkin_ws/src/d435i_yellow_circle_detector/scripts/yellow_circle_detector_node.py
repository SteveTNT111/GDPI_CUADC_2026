...
class YellowCircleDetectorNode:
    def __init__(self):
        ...
        self.depth_min_display_m = float(rospy.get_param("~depth_min_display_m", 0.01))
        self.depth_max_display_m = float(rospy.get_param("~depth_max_display_m", 2.00))
        self.depth_disagreement_ratio = float(rospy.get_param("~depth_disagreement_ratio", 0.35))
        self.close_radius_px = float(rospy.get_param("~close_radius_px", 42.0))
        self.close_distance_m = float(rospy.get_param("~close_distance_m", 0.16))
        ...
        rospy.loginfo(
            "Yellow circle detector started. target_diameter=%.3f m trusted_depth=[%.2f, %.2f] display_depth=[%.2f, %.2f] close_radius=%.1f px close_distance=%.2f m prefer_diameter=%s auto_calibrate=%s calibration_distance=%.3f m calibration_radius=%.1f px camera_info=%s body_transform=%s",
            self.target_diameter_m,
            self.depth_min_valid_m,
            self.depth_max_valid_m,
            self.depth_min_display_m,
            self.depth_max_display_m,
    ...
    def detect_from_binary(self, image, binary_mask):
        ...
        if detection is not None:
            ...
            if self.is_too_close_by_radius(detection["radius"]) or self.is_too_close_by_depth(
                raw_depth_m,
                diameter_depth_m,
                calibrated_depth_m,
            ):
                depth_m, depth_source = 0.0, "too_close"
            else:
    ...
    def is_displayable_depth(self, depth_m):
        return (
            math.isfinite(depth_m)
            and depth_m >= self.depth_min_display_m
            and depth_m <= self.depth_max_display_m
        )

    def is_too_close_by_radius(self, radius_px):
        return self.close_radius_px > 0.0 and radius_px >= self.close_radius_px

    def is_positive_depth(self, depth_m):
        return math.isfinite(depth_m) and depth_m > 0.0

    def is_too_close_by_depth(self, raw_depth_m, diameter_depth_m, calibrated_depth_m):
        if self.close_distance_m <= 0.0:
            return False

        # Prefer model-based estimates for the close-range gate. Raw depth can
        # contain short invalid spikes on object edges.
        for value in (calibrated_depth_m, diameter_depth_m):
            if self.is_positive_depth(value):
                return value < self.close_distance_m

        if self.is_positive_depth(raw_depth_m):
            return raw_depth_m < self.close_distance_m
    ...
    def choose_depth(self, raw_depth_m, diameter_depth_m, calibrated_depth_m):
        ...
        if raw_depth_m > 0.0 or diameter_depth_m > 0.0 or calibrated_depth_m > 0.0:
            rospy.logwarn_throttle(
                2.0,
                "All distance estimates are outside trusted range [%.3f, %.3f] m: raw=%.3f diameter=%.3f calibrated=%.3f. Using first positive estimate because target is not marked too close.",
                self.depth_min_valid_m,
                self.depth_max_valid_m,
                raw_depth_m,
                diameter_depth_m,
        ...
        fallback_candidates = [
            ...
            (diameter_depth_m, "diameter*"),
            (raw_depth_m, "depth*"),
        ]
        if not self.prefer_diameter_depth:
            fallback_candidates = [
                (calibrated_depth_m, "calibrated*"),
                (raw_depth_m, "depth*"),
                (diameter_depth_m, "diameter*"),
