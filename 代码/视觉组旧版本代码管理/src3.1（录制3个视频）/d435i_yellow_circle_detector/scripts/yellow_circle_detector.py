...
class YellowCircleDetector:
    def __init__(self):
        ...
        self.enable_record = get_bool_param("~enable_record", False)
    ...
    def color_callback(self, msg):
        ...
        if self.enable_record:
            self.record_frames(image, annotated, binary_mask)
    ...
    def init_recorders(self, raw_image, annotated, binary_mask):
        os.makedirs(self.record_dir, exist_ok=True)

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        raw_path = os.path.join(self.record_dir, "yellow_circle_raw_{}.avi".format(timestamp))
        annotated_path = os.path.join(self.record_dir, "yellow_circle_annotated_{}.avi".format(timestamp))
        binary_path = os.path.join(self.record_dir, "yellow_circle_binary_{}.avi".format(timestamp))

        raw_height, raw_width = raw_image.shape[:2]
        height, width = annotated.shape[:2]
        binary_height, binary_width = binary_mask.shape[:2]
        ...
        self.binary_writer = cv2.VideoWriter(
            binary_path,
            fourcc,
            self.record_fps,
        ...
        if (
            not self.raw_writer.isOpened()
            or not self.annotated_writer.isOpened()
            or not self.binary_writer.isOpened()
        ):
            self.enable_record = False
            self.release_recorders()
            rospy.logerr("Failed to open video writers. Recording disabled.")
            return

        self.record_ready = True
        rospy.loginfo("Recording raw video to: %s", raw_path)
        rospy.loginfo("Recording annotated video to: %s", annotated_path)
        rospy.loginfo("Recording binary video to: %s", binary_path)

    def record_frames(self, raw_image, annotated, binary_mask):
        if not self.record_ready:
            self.init_recorders(raw_image, annotated, binary_mask)
            if not self.record_ready:
                return
        ...
        self.raw_writer.write(raw_image)
        self.annotated_writer.write(annotated)
        self.binary_writer.write(binary_mask)

    def release_recorders(self):
