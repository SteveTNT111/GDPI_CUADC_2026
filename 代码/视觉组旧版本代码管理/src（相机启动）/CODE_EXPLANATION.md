...
Ubuntu 20.04 中源码存储位置为 `~/d435i_ws/src/`。当前 `launch/d435i_yellow_circle.launch` 已修正为只启动 `d435i_camera_node.py`。
...
源码目录中还保留检测、显示和采集脚本，方便后续恢复或单独调试；但当前 `launch` 文件只启动相机节点。
...
当前 `launch` 文件不启动此节点。如需采集图片，需要先恢复 launch 中的采集节点配置。
...
这个脚本会先检查 `~/d435i_ws`、`/opt/ros/noetic/setup.bash` 和 `devel/setup.bash` 是否存在。`roslaunch` 退出后终端不会立刻关闭, 方便查看错误日志。
...
```text
~/d435i_ws/src/d435i_yellow_circle_detector/scripts/start_d435i_record.sh
```
