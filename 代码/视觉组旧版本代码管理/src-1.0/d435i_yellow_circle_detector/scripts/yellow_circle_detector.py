...
class YellowCircleDetector:
    def __init__(self):
        ...
        self.result_pub = rospy.Publisher(result_topic, YellowCircle, queue_size=5)
        self.annotated_pub = rospy.Publisher(annotated_topic, Image, queue_size=2)
        self.depth_sub = rospy.Subscriber(depth_topic, Image, self.depth_callback, queue_size=2)
        self.color_sub = rospy.Subscriber(color_topic, Image, self.color_callback, queue_size=2)
    ...
    def color_callback(self, msg):
        ...
        detection, annotated = self.detect(image)
        ...
        if self.show_window:
            cv2.imshow("yellow_circle_detector", annotated)
            cv2.waitKey(1)
