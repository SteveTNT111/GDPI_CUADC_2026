...
import rospy
import tf2_geometry_msgs  # Registers geometry_msgs transforms for tf2 Buffer.transform.
import tf2_ros
from geometry_msgs.msg import PointStamped, PoseStamped
from sensor_msgs.msg import NavSatFix, NavSatStatus

from d435i_yellow_circle_detector.msg import GeoTarget, YoloDetection
...
def enu_offset_to_geodetic(origin_lat, origin_lon, origin_alt, east_m, north_m, up_m):
    try:
        ...
        if horizontal_m > 0.0:
            ...
            lat = origin_lat
            lon = origin_lon
        return lat, lon, origin_alt + up_m
    except Exception:
...
class TargetGeoposeNode:
    def __init__(self):
        self.detection_topic = rospy.get_param("~detection_topic", "/yolo/detection")
        ...
        self.output_topic = rospy.get_param("~output_topic", "/competition/target_global")
        self.body_frame = rospy.get_param("~body_frame", "base_link")
        self.default_camera_frame = rospy.get_param("~camera_frame", "d435i_color_optical_frame")
        ...
        rospy.loginfo(
            "Target geopose node started. detection=%s global=%s local_pose=%s output=%s body_frame=%s camera_frame=%s",
            self.detection_topic,
            self.global_position_topic,
            self.local_pose_topic,
            self.output_topic,
    ...
    def detection_callback(self, msg):
        ...
        body_x = body_point.point.x
        body_y = body_point.point.y
        body_z = body_point.point.z
        east_m, north_m, up_m = quaternion_rotate_vector(
            latest_local_pose.pose.orientation,
            (body_x, body_y, body_z),
        )
        lat, lon, alt = enu_offset_to_geodetic(
            latest_global.latitude,
            latest_global.longitude,
            latest_global.altitude,
            east_m,
            north_m,
            up_m,
        )
        ...
        target.valid = True
        target.status = "ok"
        target.body_x_m = float(body_x)
        target.body_y_m = float(body_y)
        target.body_z_m = float(body_z)
    ...
    def make_base_target(self, detection):
        ...
        target.body_y_m = 0.0
        target.body_z_m = 0.0
        target.enu_east_m = 0.0
        target.enu_north_m = 0.0
        target.enu_up_m = 0.0
        target.latitude = 0.0
        ...
        try:
            ...
            try:
                ...
                rospy.logwarn_throttle(
                    2.0,
                    "Failed to transform target from %s to %s: %s / %s",
...
def main():
    rospy.init_node("target_geopose_node")
    TargetGeoposeNode()
    rospy.spin()
